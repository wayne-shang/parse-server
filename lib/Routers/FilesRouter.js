...
var FilesRouter = exports.FilesRouter = function () {
  ...
  _createClass(FilesRouter, [{
    ...
    value: function expressRouter() {
      ...
      router.post('/wxfiles/:filename', Middlewares.allowCrossDomain, _bodyParser2.default.raw({ type: function type() {
          return true;
        }, limit: options.maxUploadSize || '20mb' }), // Allow uploads without Content-Type, or with any Content-Type.
      Middlewares.handleParseHeaders, this.wxcreateHandler);
    ...
    key: 'wxcreateHandler',
    value: function wxcreateHandler(req, res, next) {
      ...
      filesController.createFile(config, filename, data, 'multipart/form-data').then(function (result) {
        res.status(200);
